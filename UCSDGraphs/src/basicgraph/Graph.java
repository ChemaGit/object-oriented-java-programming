...
public abstract class Graph {
	 ...
	 */
	private HashMap<Integer,ArrayList<Integer>> getNeighbors(int vertex,int limit, HashMap<Integer,ArrayList<Integer>>neighbors) {
		if(vertex == limit) return neighbors;
		else {
			neighbors.put(vertex,(ArrayList<Integer>)this.getNeighbors(vertex));
			return getNeighbors(++vertex, limit, neighbors);
		}
	}

	private HashMap<Integer,ArrayList<Integer>> getInNeighbors(int vertex,int limit, HashMap<Integer,ArrayList<Integer>>neighbors) {
		if(vertex == limit) return neighbors;
		else {
			neighbors.put(vertex,(ArrayList<Integer>)this.getInNeighbors(vertex));
			return getInNeighbors(++vertex, limit, neighbors);
		}
	}

	public List<Integer> degreeSequence() {
		// TODO: Implement in part 1 of week 2
		HashMap<Integer, ArrayList<Integer>> neighbors = getNeighbors(0, this.getNumVertices(), new HashMap<Integer, ArrayList<Integer>>());
		HashMap<Integer, ArrayList<Integer>> inNeighbors = getInNeighbors(0, this.getNumVertices(), new HashMap<Integer, ArrayList<Integer>>());
		List<Integer> degreeSeq = new ArrayList<>();
		neighbors.forEach( (k, v) -> {
			degreeSeq.add(v.size() + inNeighbors.get(k).size());
		});
	...
	public static void main (String[] args) {
		...
		GraphLoader.loadRoutes("data/airports/routesUA.dat", new GraphAdjList());
