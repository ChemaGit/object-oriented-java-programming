...
public abstract class EarthquakeMarker extends SimplePointMarker {
	...
	/**If the EarthQuake occurred in the past day*/
    private static final String PAST_DAY = "Past Day";
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		if(age.equals(PAST_DAY)) {
			pg.fill(0, 0, 0); // black color
	...
	private void colorDetermine(PGraphics pg) {
		...
		if(depth >= 0 && depth < THRESHOLD_INTERMEDIATE) {
			pg.fill(255, 255, 0); // shallow -> yellow color
		} else if(depth >= 70 && depth < THRESHOLD_DEEP) {
			pg.fill(0, 0, 255); // intermediate -> blue color
		} else {
			pg.fill(255, 0, 0); // deep -> red color
		}
	}
