...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		fill(255, 0, 0); // red color
		ellipse(65, 240, 10, 10);
		fill(255, 255, 255); // white color
		ellipse(65, 260, 10, 10);
		strokeWeight(2);
...
//		}
		...
		return countryMarkers.stream().filter(m -> isInCountry(earthquake,m)).count() > 0;
	...
	private void printQuakes() {
		...
		for(Marker ea : quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)ea;
			if(em.isOnLand) {
				String country = ((LandQuakeMarker)em).getCountry();
				if(counter.containsKey(country)) {
					Integer count = counter.get(country);
					count = count + 1;
					counter.replace(country, count);
				} else {
					counter.put(country, new Integer(1));
				}
				landEarthquakes = landEarthquakes + 1;
			} else {
				oceanEarthquakes = oceanEarthquakes + 1;
			}
			totalEarthquakes = totalEarthquakes + 1;
		}
		counter.forEach( (c,n) -> System.out.println("Country: " + c + " - Quakes: " + n));
		System.out.println("Land Earth Quakes: " + landEarthquakes);
		System.out.println("Ocean Earth Quakes: " + oceanEarthquakes);
		System.out.println("Total Earth Quakes: " + totalEarthquakes);
	}
